...
export async function createPage(ctx) {
    ...
    async function onSubmit(event) {
        ...
        if(description.length<10) {
            return alert('The title should be at least 10 characters long');
        }
        if(img.length<5) {
            return alert('The title should be at least 5 characters long');
        }

        await createRecord({title, description, img});
        ctx.setUserNav();
